 ...
 */
function getOptions (user = undefined) {
	var ops = {
	...
	_.forEach(this.uiElements, function (el) {
		switch (el.type) {
			...
			case 'field':
				...
				// user.roleがあればロールに合わせたオプションを返す
				ops.fields[el.field.path] = (user) ? el.field.getUserOptions(user) : el.field.getOptions();

				// don't output hidden fields
				if (el.field.hidden && ops.fields[el.field.path].hidden) {
					return;
				}
	...
	// リクエストユーザロールに合わせてオプションを上書き
	if (user && this.options.listPermissions && _.isObject(this.options.listPermissions[user.role])) {
		var overrideOptions = this.options.listPermissions[user.role];
